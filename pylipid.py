    ...
    def cal_interactions(self, save_dir=None, plot_koff=True, save_dataset=True):
        ...
        if plot_koff:
            ...
            for residue in self.residue_set:
                ...
                if np.sum(duration_raw) > 0 and len(duration_raw) > 20:
                    graph_koff(durations_raw, self.sigma[residue], self.koff[residue], self.A[residue], self.timeunit, residue, "{}/{}_{}.tiff".format(koff_dir, self.lipid, residue))
        ...
        dataset = pd.DataFrame({"Residue": [residue for residue in self.residue_set],
                                ...
                                "Duration raw_std": np.array([np.std(np.concatenate(self.interaction_duration_raw[residue])) \
                                                          for residue in self.residue_set]),
                                "Duration corrected": np.array([np.concatenate(self.interaction_duration[residue]) \
                                                                for residue in self.residue_set]),
